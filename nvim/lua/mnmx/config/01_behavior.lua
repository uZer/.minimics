...
if is_wsl() then
  -- Prefer fast persistent bridge
  if vim.fn.executable("clipipe") == 1 then
    vim.g.clipboard = {
      name = "clipipe-copy-only",
      copy = {
        ["+"] = "clipipe copy",
        ["*"] = "clipipe copy",
      },
...
elseif is_ssh() or vim.fn.has("clipboard") == 0 then
  -- SSH or no provider → OSC52
